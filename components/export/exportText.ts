...
export type ExportTextData = {
  geberName: string;
  personen: Person[];
  notes?: string;
  doc?: jsPDF; // 👈 NEU, optional
};
...
/* =========================
   PDF ERZEUGEN / ERWEITERN
   ========================= */

export async function exportKontaktbogenTextPDF(
  data: ExportTextData
): Promise<void> {
  const { geberName, personen, notes } = data;

  const OVB_BLUE = "#013F72";

  // ✅ ENTWEDER bestehendes PDF verwenden
  // ✅ ODER neues erzeugen (wie bisher)
  const doc =
    data.doc ??
    new jsPDF({
  ...
  // ❗ Nur speichern, wenn wir selbst das PDF erzeugt haben
  if (!data.doc) {
